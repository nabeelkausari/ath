...
const initialState = {
  ...
  shareable_link_requested: null,
  shareable_link_succeeded: null,
  shareable_link_failed: null,
  shareable_link: null,
  shared_dashboard_requested: null,
  shared_dashboard_succeeded: null,
  shared_dashboard_failed: null,
  shared_dashboard: [],
};

const collaboratorsReducer = (state = initialState, { type, payload }) => {
  switch (type) {
    ...
    case types.FETCH_COLLABORATORS_SUCCEEDED:
      return {
        ...
        list: [
          ...payload.map((user) =>
            state.all_users.find((i) => i.mapping_id === user.collaboratorId)
          ),
        ],
    ...
    case types.FETCH_USER_REQUESTED:
      return {
        ...state,
        fetch_user_requested: true,
        fetch_user_succeeded: false,
        fetch_user_failed: false,
      };
    ...
    case types.FETCH_USER_SUCCEEDED:
      return {
        ...state,
        all_users: [payload],
        fetch_user_requested: false,
        fetch_user_succeeded: true,
        fetch_user_failed: false,
      };
